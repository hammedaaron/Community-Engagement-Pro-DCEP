...
const CreateProfileModal: React.FC<CreateProfileModalProps> = ({ onClose, onSubmit }) => {
  const { currentUser, theme, cards, selectedFolderId, folders, activeParty, showToast } = useApp();
          ...
          {(reachedFolderLimit || reachedRateLimit) ? (
            ...
            <div className="space-y-6">
              <div className="space-y-2 relative">
                <label className="text-[11px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Display Name</label>
              ...
              {isDev && (
                <div className="p-6 rounded-[2rem] bg-emerald-500/5 border border-emerald-500/20 space-y-4 animate-in fade-in slide-in-from-top-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => setIsPermanent(!isPermanent)} className={`w-12 h-6 rounded-full transition-all relative border ${isPermanent ? 'bg-emerald-500 border-emerald-400' : 'bg-slate-700 border-slate-600'}`}><div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${isPermanent ? 'left-7' : 'left-1'}`} /></button>
                  </div>
                </div>
              )}
